...
<div class="container">
  <div class="row">
    ...
    <div class="col-6">
      <input type="text" class="form-control" name="fournisseur" id="fournisseur" value="{{$demande->fournisseur->nom_fournisseur}}" disabled>
    </div>
  </div>
  <br>
  <!-- Begin payement  -->
    ...
    <div class="card-body">
      <div class="table-responsive">
        <table class="table" id="table">
          ...
          <tbody>
            <tr>
                <td id="cmd_id" style="display: none">{{$demande->id}}</td>
                <td>{{$demande->code}}</td>
                <td>{{$demande->date}}</td>
                {{-- <td id="fournisseur">{{$demande->fournisseur->nom_fournisseur}}</td> --}}
                <td>{{$demande->facture}}</td>
                <td id="total">{{number_format($demande->total, 2, '.', '')}}</td>
                <td id="avance1">{{number_format($demande->avance, 2, '.', '')}}</td>
                <td id="reste1">{{number_format($demande->reste, 2, '.', '')}}</td>
                <td id="avance2">
                  <input type="number" 
                    min="0" 
                    style="width: 50%" 
                ...
                <td id="reste2">{{number_format($demande->reste, 2, '.', '')}}</td>
                <td id="status2">NR</td>
            </tr>
          </tbody>
          <tfoot>
  ...
  <div class="text-right">
    <button class="btn btn-secondary" id="valider">Effectuer le règlements</button>
  </div>
  <br>
</div>
<!-- ---------  BEGIN SCRIPT --------- -->
  ...
  $(document).ready(function(){
    ...
    $(document).on('click','#valider',function(e){
      ...
      var array = [];
        ...
        // var fournisseur = list.eq(i).find('td').eq(3).html();
        ...
        var fournisseur = $('#fournisseur').html();
